...
function validateUnder(block, matrix) {
    let isValid = true;

    block.shape.some((row, dy) => {
        row.some((value, dx) => {
            if(value > 0) {
                if(matrix[block.y+dy][block.x+dx] > 0) {
                    isValid = false;
                    return true;
                }
            }
...
function validRotate(block, matrix) {
    const cloneBlock1 = clone(block);
    const cloneBlock2 = clone(block);
    rotate(cloneBlock1);
    if(validate(cloneBlock1, matrix)) {
        rotate(block);
        return true;
    } else {
        return false;
